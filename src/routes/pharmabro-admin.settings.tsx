...
const NAV: { group: string; items: { to: string; label: string; icon: typeof Building2 }[] }[] = [
  { group: "Account", items: [
    { to: "/pharmabro-admin/settings/general",      label: "General",       icon: Building2 },
    { to: "/pharmabro-admin/settings/plan-billing", label: "Plan & Billing",icon: CreditCard },
    { to: "/pharmabro-admin/settings/team",         label: "Team",          icon: Users },
  ]},
  { group: "Operations", items: [
    { to: "/pharmabro-admin/settings/pharmacy-routing", label: "Pharmacy Routing", icon: Truck },
  ...
  { group: "Compliance", items: [
    { to: "/pharmabro-admin/settings/compliance", label: "Compliance & HIPAA", icon: ShieldCheck },
    { to: "/pharmabro-admin/settings/legal",      label: "Legal & Policies",   icon: Scale },
  ]},
];
          ...
          <nav className="rounded-2xl border border-ink/8 bg-white p-2 shadow-[0_1px_0_rgba(23,23,23,0.02)]">
            {NAV.map((group, gi) => (
              <div key={group.group} className={gi > 0 ? "mt-2 border-t border-ink/6 pt-2" : ""}>
                ...
                {group.items.map((it) => {
                  const active = pathname === it.to || (it.to === "/pharmabro-admin/settings/general" && pathname === "/pharmabro-admin/settings");
                  const Icon = it.icon;
                  return (
