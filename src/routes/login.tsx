...
import { actions as patientActions } from "@/lib/portal/store";

export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Sign in — Blissley" },
      { name: "description", content: "Secure magic-link sign in to your Blissley account." },
...
function PatientLogin() {
  ...
  const enterDemo = () => {
    patientActions.signIn(email);
    nav({ to: "/portal/patient" });
  };
  ...
  return (
    <main className="min-h-screen bg-canvas text-ink">
      <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-10">
        ...
        <h1 className="font-hero text-3xl tracking-tight text-ink">Sign in</h1>
        ...
        <div className="mt-10 text-xs text-ink/50">
          Are you a physician? <Link to="/login/physician" className="underline">Sign in here</Link>.
        </div>
      </div>
    </main>
  );
