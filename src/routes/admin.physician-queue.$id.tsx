...
export const Route = createFileRoute("/admin/physician-queue/$id")({
  head: ({ params }) => ({ meta: [
...
function CaseDetailPage() {
  const { id } = useParams({ from: "/admin/physician-queue/$id" });
    ...
    return (
      <AdminShell title="Case">
        <Card className="p-8 text-center">
          ...
          <Link to="/admin/physician-queue" className="mt-3 inline-block text-[13px] font-semibold text-marine hover:underline">Back to queue</Link>
        </Card>
      </AdminShell>
    );
  }
  ...
  return (
    <AdminShell title="Case">
      <button onClick={() => nav({ to: "/admin/physician-queue" })}
