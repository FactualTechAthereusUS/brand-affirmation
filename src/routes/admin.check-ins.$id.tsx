...
export const Route = createFileRoute("/admin/check-ins/$id")({
  head: ({ params }) => ({ meta: [
...
function CheckInDetailPage() {
  const { id } = useParams({ from: "/admin/check-ins/$id" });
    ...
    return (
      <AdminShell title="Check-in">
        <Card className="p-8 text-center">
          ...
          <Link to="/admin/check-ins" className="mt-3 inline-block text-[13px] font-semibold text-marine hover:underline">Back</Link>
        </Card>
      </AdminShell>
    );
  }
  ...
  return (
    <AdminShell title="Check-in">
      <button onClick={() => nav({ to: "/admin/check-ins" })}
        ...
        <div className="space-y-3 lg:sticky lg:top-20 lg:self-start">
          ...
          {patient && (
            <Card className="p-4">
              ...
              <Link to="/admin/patients/$id" params={{ id: patient.id }}
          ...
          {ci.refillOrderId && (
            <Card className="p-4">
              ...
              <Link to="/admin/orders/$id" params={{ id: ci.refillOrderId }}
