...
import { fetchCollectionProducts, money, productImages, variantsOf } from "@/lib/uo/shopify";
import { useUOCart } from "@/lib/uo/cart";

/**
 * Section 8 — Alo's shoppable UGC carousel with @handles, wired to the live
 * catalog: each card links to a real PDP and can add the item to the cart.
 */
export function UOUgc() {
  const { addLine, busy } = useUOCart();
  const { data } = useQuery({
    queryKey: ["uo", "collection", "best-selling-products", 8],
    queryFn: () => fetchCollectionProducts("best-selling-products", 8),
    staleTime: 5 * 60_000,
  });
  const products = data ?? [];
        ...
        <Rise delay={0.08} className="mt-7 md:mt-10">
          <Rail contentClassName="-mx-4 px-4 md:mx-0 md:px-0">
            {UGC.map((u, i) => {
              ...
              const photo = product ? productImages(product)[1]?.url ?? productImages(product)[0]?.url : null;
              const variant = product ? variantsOf(product).find((v) => v.availableForSale) : null;
              return (
                <figure
                  key={u.handle}
                ...
                >
                  <div className="relative overflow-hidden bg-[#efe9dd]">
                    {product ? (
                      <Link to="/unhingedone/product/$handle" params={{ handle: product.handle }}>
                        {photo ? (
                          <img
                            src={photo}
                    ...
                    <button
                      type="button"
                      aria-label={product ? `Add ${product.title} to cart` : "Shop"}
                    ...
                    {product ? (
                      <div className="absolute inset-x-2 bottom-2 translate-y-3 bg-canvas/95 p-2.5 opacity-0 backdrop-blur transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                        <p className="line-clamp-2 text-[11px] font-medium leading-snug">{product.title}</p>
