...
export function OperatorShell({ children, title, description, action }: { children: ReactNode; title: string; description?: string; action?: ReactNode }) {
  ...
  return <div className="admin-scope min-h-screen bg-canvas text-ink"><aside className="fixed inset-y-0 left-0 z-30 hidden w-56 flex-col border-r border-ink/10 bg-white lg:flex">{sidebar}</aside>{open&&<><button aria-label="Close navigation" className="fixed inset-0 z-40 bg-ink/35 lg:hidden" onClick={()=>setOpen(false)}/><aside className="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-white shadow-xl lg:hidden">{sidebar}</aside></>}<div className="lg:pl-56"><header className="sticky top-0 z-20 flex h-14 items-center border-b border-ink/10 bg-white px-4 lg:px-6"><Button variant="ghost" size="icon" className="lg:hidden" onClick={()=>setOpen(true)}><Menu/></Button><div className="ml-auto flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-check"/><span className="text-xs text-ink/55">Platform operational</span></div></header><main className="px-4 py-5 lg:px-7"><div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-2xl font-semibold text-ink">{title}</h1>{description&&<p className="mt-1 text-sm text-ink/55">{description}</p>}</div>{action}</div>{children}</main></div></div>;
}
